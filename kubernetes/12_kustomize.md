...
│   ├── kustomization.yaml
│   └── kafka-cluster.yaml
└── overlays/     # 환경에 따라 다른 부분 구성. dev/prod/svc1/svc2와 같이 구분가능 
    ├── dev/
    │   ├── kustomization.yaml
    │   └── patch-replicas.yaml
    └── prod/
...
### 2. base/kustomization.yaml (공통 설정 예시)
...
# 모든 리소스에 표준 라벨 주입
# 배포하는 모든 yaml에 공통으로 주입됨
# 이를 통해서 Helm 차트처럼 여러 yaml을 하나의 APP처럼 관리 가능 
commonLabels:
...
### 3. overlays/prod/kustomization.yaml (운영 전용 예시)
...
# 운영 환경 전용 식별 라벨 추가 (Helm의 Release Name 역할)
commonLabels:
  app.kubernetes.io/instance: prod-v1
  env: production

# 운영 환경에서만 이미지 태그 변경 (수치 지정)
images:
  - name: my-app-image
    newTag: v1.2.3

# 특정 리소스 값 수정 (예: 복제본 5개로 증설)
patches:
  - target:
      kind: Deployment
      name: my-app
