...
- 업그레이드시, 콘솔 클러스터 메뉴의 `업그레이드 인사이트` 항목이 모두 Passing 상태여야 한다.
  ...
  - 업그레이드 인사이트에서 ERROR 항목이 있을 시 자동업데이트도 되지 않는다.
...
- 또한 이러한 유형의 앱들은 신규노드 배포시에도 영향을 끼칠 수 있으므로,
- **수동설치된 컨트롤러 앱들과, 신규버전 Kubernetes의 호환성을 반드시 체크**해야 한다.

### 현재 실행된 노드AMI가 차기버전 kubernetes와 미호환되는 경우

- AWS 노드그룹으로 배포된 노드는
  - 마이너 버전 업글은 콘솔에서 마우스 클릭 몇번으로 쉽게 가능하며, 롤링업데이트 등 간단한 전략적 업데이트 방안이 제공된다.
  - 메이저 버전 또는 다른유형AMI 업글은 노드그룹을 삭제하고 새로 생성하거나, 기존 시작템플릿의 신규버전 생성 필요(AMAZON LINUX 2 => 2023 처럼 세대가 다른 경우, 보통 시작템플릿 변경으로도 해결이 안되고 완전 새로운 노드그룹 생성 필요. snapshotId cannot be modified on root device)
