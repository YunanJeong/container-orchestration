...
- Karpenter 등등
- 컨트롤플레인 또는 노드배포 영역에 관여하는 앱들을 Helm 등으로 수동배포 후 클러스터를 관리하는 경우가 많은데, 개별 설치한 것이므로 EKS 시스템 내에서 컨트롤플레인 영역으로 인식하지 않는다. 업그레이드 인사이트에서 확인불가.
- **수동설치된 컨트롤러 앱들과, 신규버전 Kubernetes의 호환성을 반드시 체크**해야 한다.

### 현재 실행된 노드AMI가 차기버전 kubernetes와 미호환되는 경우

- 콘솔에서 마우스 클릭 몇번으로 업데이트가 쉽게 가능하다.
- 당연히 실행중인 노드가 꺼지므로, 일반적인 쿠버네티스 앱이라면 상관없겠지만
- 항상 켜져있어야 하거나, Stateful한 앱이 있을 수 있으므로 이 부분을 체크하자!
  - 원칙적으로는 이런 앱들은 EBS기반 PV, StatefulSet, 클러스터링 기반 HA 처리가 미리 되어있어야하는게 맞다.
