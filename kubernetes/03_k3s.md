...
- 각 서버마다 `다중 IP가 할당`된 상황이며, 별도 설정이 없을시 K3s는 기본 사설망(eth0)을 default로 선택한다.
- 이를 `고대역폭망 IP(eth1)로 변경`하여 클러스터 내외부 트래픽이 송수신되도록 해야 함

### 설정 필요한 부분

- 각 노드에서 가리키는 K3s Api Server의 주소를 eth1로 설정(:6443)
- 각 노드의 internal ip를 eth1로 설정(:8472, :10250)
  - 각 노드가 클러스터 내부에서 자기자신을 인식하는 주소를 의미
  - 노드마다 다르게 실제 eth1으로 할당된 주소를 설정해줘야 함
- NIC를 eth1로 설정(:8472)
  - 앞 과정에서 internal ip를 설정했음에도 불구하고 eth0으로 트래픽전송이 시도됨
  - eth1을 수동선택 해줘야 함
- external ip
