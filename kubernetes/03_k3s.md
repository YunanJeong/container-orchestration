...
### K3s Server 추가 설치 (Controlplane+Node)

추가하는 Server도 컨트롤플레인+노드 한 세트다. 여러 대를 두면 컨트롤플레인이 HA가 되고, 노드도 그만큼 늘어난다. (원리·쿼럼·운영은 아래 `## 컨트롤플레인 HA 구성` 참고)

```sh
# 1번째 Server: --cluster-init으로 임베디드 etcd 모드로 클러스터 초기화
curl -sfL https://get.k3s.io | sh -s - server --cluster-init

# 2번째 이후 Server: 기존 클러스터에 join
# Server join은 K3S_TOKEN 환경변수 + --server 플래그로 1번째 Server를 가리킴 (Agent와 달리 K3S_URL 안 씀)
# {k3s_server_token}은 1번째 Server에서 조회: sudo cat /var/lib/rancher/k3s/server/node-token
export K3S_TOKEN={k3s_server_token}
curl -sfL https://get.k3s.io | sh -s - server --server https://{k3s_server_ip}:6443

# 추가 Server 설치 (한 번에 입력)
curl -sfL https://get.k3s.io | K3S_TOKEN={k3s_server_token} sh -s - server --server https://{k3s_server_ip}:6443
```
...
- 단일 Server = 데이터스토어가 SQLite = 컨트롤플레인 1개 = SPOF.
- 다중 Server = 임베디드 etcd = 컨트롤플레인 다중화 = 1대 죽어도 클러스터 관리 유지.
- 참고로 일반 K8s는 etcd가 기본이지만, K3s는 경량 지향이라 기본값이 SQLite다. HA(다중 Server)를 하려면 `--cluster-init`으로 etcd를 의도적으로 켜야 한다. (컨트롤플레인을 단일로 둘지 다중으로 세울지의 선택이며, 워커 노드 수와는 무관)

### 구성 방법

- 첫 Server를 `--cluster-init`으로 시작하면 임베디드 etcd 모드가 켜진다. 이후 Server를 `--server`로 join한다. (설치 명령은 위 `### K3s Server 추가 설치` 참고)
- 홀수(3, 5 …) 로 구성한다 (아래 쿼럼 참고).
- K3s Server는 controlplane + 워커를 겸한다 (K3s는 기본 taint가 없어 Server 노드에도 앱이 스케줄됨).

### 쿼럼 (etcd 동작 원리)

- etcd는 과반(쿼럼) 합의 구조다. 쿼럼 = (전체/2)+1.
- 과반을 확보한 쪽만 동작하고, 과반을 못 넘기면 스스로 멈춘다 → 네트워크가 갈라져도 양쪽이 각자 다른 데이터를 쓰는 사태(split-brain)를 원천 차단.
...
- etcd는 멤버 목록을 명시적으로 기록한다. 노드가 죽어도 멤버에서 자동으로 빠지지 않으므로, 죽은 채 방치하면 "지금은 살아있지만 장애 내성 0"인 상태가 된다. (`kubectl get nodes` 목록과는 별개 계층)
...
- 컨트롤플레인 마비 ≠ 워크로드 정지. 쿼럼이 깨져 컨트롤플레인이 멈춰도, 이미 떠 있는 파드는 각 노드의 kubelet이 계속 실행한다. 멈추는 것은 관리 기능(새 배포·스케줄링·조회·죽은 파드 자가복구)이다.
